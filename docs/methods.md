...
The separate sitewide `artist-evolution-activity?range=this_week` endpoint returns weekday artist counts for a **limited top-artist subset** (it supplied about 20 artists per day when inspected). These optional rows appear in artist details. A missing row is *unknown*, not zero. We do not manufacture 100 daily counts from this incomplete table. The main node size changes when a new, distinct sitewide artist chart calculation is archived. Within the same UTC week, `change_since_previous_snapshot` is the difference between those two source calculations; a delayed upstream run may span multiple days. A rollover to a new week resets this comparison to unavailable rather than subtracting a full and partial week.
